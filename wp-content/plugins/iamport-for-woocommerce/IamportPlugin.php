...
function iamport_vbank_order_details($order) {
	$pay_method = get_post_meta($order->id, '_iamport_paymethod', true);
	$vbank_name = get_post_meta($order->id, '_iamport_vbank_name', true);
	$vbank_num = get_post_meta($order->id, '_iamport_vbank_num', true);
	$vbank_date = get_post_meta($order->id, '_iamport_vbank_date', true);
...
function iamport_order_detail_in_history( $order ) {
	$pay_method = get_post_meta($order->id, '_iamport_paymethod', true);
	$pg_provider = get_post_meta($order->id, '_iamport_provider', true);
	
	$gateway = find_gateway($pg_provider, $pay_method);
	if ( $gateway ) {
		$gateway->iamport_order_detail($order->id);
	}
}
...
function iamport_valid_order_statuses_for_cancel($statuses, $order=null) {
	...
	$rest_key = get_post_meta($order->id, '_iamport_rest_key', true);
	$rest_secret = get_post_meta($order->id, '_iamport_rest_secret', true);
...
function woocommerce_gateway_iamport_init() {
	...
	abstract class Base_Gateway_Iamport extends WC_Payment_Gateway {
		...
		public function generate_order_key($order_key) {
			return 'orderx_'.rand(0, 99999).substr( preg_replace( "/[^A-Za-z0-9_]/", '', uniqid('', true)), 10 ); //more entropy
		}
