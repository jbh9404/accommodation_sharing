...
class WC_Gateway_Iamport_Subscription_Ex extends Base_Gateway_Iamport {
	...
	protected function get_gateway_id() {
		return 'iamport_subscription_ex';
	}
	...
	public function iamport_payment_info( $order_id ) {
		...
		if ( !$this->isEmptyProvider() && !empty($this->pg_id) ) $iamport_info['pg'] = $this->pg_provider.'.'.$this->pg_id;
		$iamport_info['customer_uid'] = $customer_uid; //js에 전달

		if ( in_array($this->pg_provider, array('jtnet', 'danal_tpay')) || ($this->isMobile() && $this->pg_provider === 'html5_inicis') ) {
			$iamport_info['amount'] = 0; //다날-휴대폰, 이니시스-PC결제의 경우 원래 가격 그대로 설정
		}
	...
	public function is_available() {
		return !empty( WC()->cart->recurring_carts );
	}
	...
	*/
	public function check_payment_response() {
		...
		if ( !empty($http_param['imp_uid']) ) {
			...
			if ( $result->success ) {
				...
				if ( $this->isAlreadyPaidOrder($order_id) )		return wp_redirect( $order->get_checkout_order_received_url() );
				...
				if ( $payment_data->status === 'paid' ) {
					...
					if ( $payment_data->pg_provider === 'danal' ) { //다날-휴대폰 소액결제 정기결제는 이미 결제를 하고 돌아옴
					...
					if ( is_wp_error( $response ) || !($response instanceof WooIamportPayment) ) {
						...
						try {
							...
							if ( !$this->has_status($synced_row->post_status, array('processing', 'completed')) ) {
								...
								//fire hook
								do_action('iamport_order_status_changed', $synced_row->post_status, $order->get_status());
			...
			$called_from_iamport ? exit('IamportForWoocommerce 1.7.6') : wp_redirect( $default_redirect_url );
		} else {
			//just test(아임포트가 지원하는대로 호출되지 않음)
			exit( 'IamportForWoocommerce 1.7.6' );
		}
	}
	...
	private function doPayment($creds, $order, $total, $customer_uid, $number_of_tried = 0) {
		...
		$pay_data = array(
			'amount' => $total,
			'merchant_uid' => $order->order_key . $order_suffix,
	...
	private function isAlreadyPaidOrder($order_id) {
		...
		try {
			...
			$alreadyPaid = $this->has_status($synced_row->post_status, array('processing', 'completed'));
		...
		if ( is_wp_error( $response ) ) {
			...
			try {
				...
				if ( !$this->has_status($synced_row->post_status, array('processing', 'completed')) ) {
					...
					//fire hook
					do_action('iamport_order_status_changed', $synced_row->post_status, $renewal_order->get_status());
