	...
	function mbw_get_admin_template($mode, $data){
		...
		if($item_type=='admin_board_name'){
			...
			if($board_type=="admin"){			
				...
				if($board_type=="link" || $board_type=="custom") $board_type		= "board";
			...
			if(!empty($board_name) && $board_name!="N"){
				...
				$link_url			= admin_url('admin.php')."?page=mbw_board_options&board_name=".str_replace($mb_table_prefix,"", mbw_get_board_item("fn_table_name"));
