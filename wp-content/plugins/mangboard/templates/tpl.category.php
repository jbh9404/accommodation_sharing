	...
	function mbw_get_category_template($type, $data=""){
		...
		// 카테고리를 탭메뉴로 보여주기
			...
			foreach($tab_menu as $value){
				if($value==mbw_get_param("category1")) $tab_menu_class			= "tab-menu-on";
				else $tab_menu_class			= "tab-menu-off";						
				if($type=="TAB_RELOAD"){
					$template_category	.= mbw_get_btn_template(array("name"=>$value,"onclick"=>"sendTabReload('".rawurlencode($value)."')","class"=>$tab_menu_class));
				}else if($type=="TAB_AJAX"){
					$template_category	.= mbw_get_btn_template(array("name"=>$value,"onclick"=>"selectTabMenu(this,'".($value)."')","class"=>$tab_menu_class));
				}
			}
