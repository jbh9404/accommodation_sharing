	...
	function mbw_get_input_template($mode, $data){
		...
		if(strpos($item_type,'file')===0){
			if($item_type=='file_download'){
				...
				if(!empty($img_path)) $template_start	= '<img'.$data["ext"].__STYLE("max-width:".$data["img_width"].";max-height:".$data["img_height"].";margin-right:10px !important;").' src="'.$img_path.'" />';

				if($item_type=='file_singular_upload'){
					$upload_type		= ' accept="image/*"';
					$template_start	.= '<input'.$data["ext"].__STYLE("width:".$data["width"].";".$data["style"]).' name="'.mbw_set_form_name($data["item_name"]).'" id="'.$data["item_id"].'" '.$upload_type.' type="file"/>';
				...
				if(mbw_get_param("board_action")=="modify"){
					...
					if(!empty($file)){
						$template_start	= '<input'.$data["ext"].__STYLE($data["style"]).' name="'.mbw_set_form_name("file_delete_pid[]").'" id="'.$data["item_id"].'" value="'.$file[$mb_fields["files"]["fn_pid"]].'" title="'.$file[$mb_fields["files"]["fn_file_name"]].'" type="checkbox"/>';
				...
				if(empty($template_start)){
					...
					$template_start	= '<input'.$data["ext"].__STYLE("width:".$data["width"].";".$data["style"]).' name="'.mbw_set_form_name($data["item_name"]).'" id="'.$data["item_id"].'" value="'.$data["value"].'"'.$file_type.' type="file"/>';
				}
			}
			...
			if(!empty($data["value"])){
				...
				if(!empty($img_url)){
					...
					if($item_type=='img'){						
						...
						$template_start	= '<div'.$data["ext"].__STYLE("max-width:".$data["width"].";width:".$data["width"].";height:".$data["height"].";margin:0 auto;background-image:url(".$img_url.");background-position:".$data["background-position"].";background-size:".$data["background-size"].";".$data["style"]).' ></div>';					
					}
				}
			}else{
				if(empty($data["width"])) $data["width"]			= "50px";
			...
			if($item_type=='textarea'){
				...
				$template_start	= '<textarea'.$data["ext"].__STYLE("width:".$data["width"].";height:".$data["height"].";".$data["style"]).' name="'.mbw_set_form_name($data["item_name"]).'" id="'.$data["item_id"].'">'.$data["value"].'</textarea>';
			}else if($item_type=='text_static'){
				$template_start	= '<span>'.$data["value"].'</span><input'.$data["ext"].__STYLE("width:".$data["width"].";".$data["style"]).' name="'.mbw_set_form_name($data["item_name"]).'" id="'.$data["item_id"].'" value="'.($data["value"]).'" type="hidden" />';
			}else{
				$add_attribute		= "";
				...
				$template_start	= '<input'.$data["ext"].__STYLE("width:".$data["width"].";".$data["style"]).' name="'.mbw_set_form_name($data["item_name"]).'" id="'.$data["item_id"].'" value="'.($data["value"]).'" type="text"'.$add_attribute.' />';
			}				
			...
			if(empty($data["width"])) $data["width"]			= "90%";
			$template_start	= '<input'.$data["ext"].__STYLE("width:".$data["width"].";".$data["style"]).' name="'.mbw_set_form_name($data["item_name"]).'" id="'.$data["item_id"].'" value="" type="password" />';

		}else if($item_type=='hidden'){
			if($data["value"]!="" && (strpos($data["value"],"'")!==false || strpos($data["value"],'"')!==false))
					$data["value"]			= mbw_htmlspecialchars($data["value"]);

			$template_start	= '<input'.$data["ext"].__STYLE("width:".$data["width"].";".$data["style"]).' name="'.mbw_set_form_name($data["item_name"]).'" id="'.$data["item_id"].'" value="'.($data["value"]).'" type="hidden" />';		
		
		//html5
		}else if($item_type=='url' || $item_type=='tel' || $item_type=='search' || $item_type=='email' || $item_type=='color'){
			$template_start	= '<input'.$data["ext"].__STYLE("width:".$data["width"].";".$data["style"]).' name="'.mbw_set_form_name($data["item_name"]).'" id="'.$data["item_id"].'" value="'.$data["value"].'" type="'.$item_type.'" />';
			...
			$template_start	= '<input'.$data["ext"].__STYLE("width:".$data["width"].";".$data["style"]).' name="'.mbw_set_form_name($data["item_name"]).'" id="'.$data["item_id"].'" value="'.$data["value"].'" type="'.$item_type.'"  min="'.$data["min"].'" max="'.$data["max"].'" step="'.$data["step"].'" />';
			...
			$template_start	= '<select'.$data["ext"].__STYLE($data["style"]).' name="'.mbw_set_form_name($data["item_name"]).'" id="'.$data["item_id"].'">';			
			$item_check		= false;
			$count				= count($t_data);
			...
			$template_start	= make_clickable($data["value"]);
