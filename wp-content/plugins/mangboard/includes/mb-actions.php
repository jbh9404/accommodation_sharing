	...
	function mbw_head_meta(){
		if(mbw_get_trace("mbw_head_meta")==""){
			...
			if(!empty($mb_fields["select_board"]) && mbw_get_param("mode")=="view"){
				...
				$select_field		= implode(",",array_values($mstore->get_board_select_fields(array("fn_title","fn_content","fn_image_path","fn_user_name","fn_tag","fn_reg_date","fn_modify_date"))));
			...
			if(mbw_get_option("use_seo")){
				...
				if(!empty($published_time)){
					echo '<meta name="article:published_time" content="'.esc_attr($published_time).'" />'.PHP_EOL;
				}

				if(!empty($updated_time) && $published_time!=$updated_time){
					echo '<meta name="article:modified_time" content="'.esc_attr($updated_time).'" />'.PHP_EOL;
					echo '<meta name="og:updated_time" content="'.esc_attr($updated_time).'" />'.PHP_EOL;
				}
				if(!empty($image_path)){
					echo '<meta property="og:image" content="'.esc_attr($image_path).'" />'.PHP_EOL;
					echo '<meta property="twitter:image" content="'.esc_attr($image_path).'" />'.PHP_EOL;
				}
				if(!empty($description)){
...
if(!function_exists('mbw_filter_header_title')){	
	function mbw_filter_header_title($w_title){
		...
		if(!empty($mb_fields["select_board"]) && mbw_get_param("mode")=="view" && mbw_get_param("board_pid")!=""){
			...
			if(!empty($mb_fields["select_board"]["fn_title"])){
