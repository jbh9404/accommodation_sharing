...
if(!function_exists('mbw_get_hash_key')){
	function mbw_get_hash_key($tag,$expiration=0,$uid="",$token=""){		
		...
		if($tag=="nonce" && mbw_get_board_name()!=""){
			...
			$table_name		= str_replace($mb_table_prefix, "", $table_name);
...
if(!function_exists('mbw_verify_nonce')){
	function mbw_verify_nonce(){
		...
		if(mbw_get_param("mb_nonce_value")!="" && mbw_get_param("mb_nonce_time")!=""){
			$hash				= mbw_get_hash_key("nonce",mbw_get_param("mb_nonce_time"));
			if(mbw_get_param("mb_nonce_value")==$hash) return true;		
			else if(mbw_is_admin_page()) return true;
		}
		return false;	
	}
}
if(!function_exists('mbw_create_nonce')){
	function mbw_create_nonce($type){
		...
		if($type=="form"){
			...
			$result			= $result.'<input type="hidden" name="mb_nonce_time" value="' . $time . '" />';
		}else if($type=="param"){
			$result			= "mb_nonce_value=".$nonce."&mb_nonce_time=".$time;
		}		
		return $result;
	}
}
...
if(!function_exists('mbw_set_board_params')){
	function mbw_set_board_params($args=NULL){
		if(mbw_get_trace("mbw_set_board_params")==""){
			...
			if(isset($args)){			
				if(empty($board_name) && isset($args['name'])) $board_name		= $args['name'];
...
if(!function_exists('mbw_get_board_table_name')){
	function mbw_get_board_table_name($board_name,$mode="board",$type=""){
		$name			= $board_name;
		if(mbw_get_board_option("fn_table_link")!="") $name	= mbw_get_board_option("fn_table_link");
...
if(!function_exists('mbw_init_options')){
	function mbw_init_options($board_name){		
		...
		if($mb_board_table_name!="none" && !$mstore->table_exists($mb_board_table_name)){
			...
			if(!empty($select_fields[$date_field])){
				if(mbw_get_param("search_date")!=""){
					...
					if(mbw_get_param("end_date")!="" && mbw_get_param("end_date")!=date("Y-m-d",mbw_get_timestamp()))
						mbw_set_board_where(array("field"=>$date_field, "value"=>mbw_get_param("end_date")." 23:59:59", "sign"=>"<="));
			...
			if(!empty($select_fields[mbw_get_param("search_field")]) && mbw_get_param("search_text")!=""){
				mbw_set_board_where(array("field"=>mbw_get_param("search_field"), "value"=>"%".mbw_htmlspecialchars(mbw_get_param("search_text"))."%", "sign"=>"like"));
			}
...
if(!function_exists('mbw_init_javascript')){
	function mbw_init_javascript(){
		global $wp_scripts,$mb_vars,$mb_api_urls,$mb_words;
		...
		wp_localize_script( 'assets-js-common-js', 'ajax_object', array( 'ajax_url' => $admin_ajax_url,'admin_page' => (is_admin()? "true":"false")));		//차후 삭제 예정
...
if(!function_exists('mbw_print_scripts')){
	function mbw_print_scripts(){
		if(mbw_get_trace("mbw_print_scripts")==""){
...
if(!function_exists('mbw_print_styles')){
	function mbw_print_styles(){
		if(mbw_get_trace("mbw_print_styles")==""){
...
if(!function_exists('mbw_get_move_script')){
	function mbw_get_move_script($type){
		...
		return '<script type="text/javascript">'.$move_script.'</script>';
	}
}
	...
	function mbw_check_cookie($data){
		...
		if($data["save"]=="db"){
			...
			if(strpos($cookie_value,$check_value)===false){			
				mbw_set_cookie($data["type"], $cookie_value.$check_value.",");
				return "success";
			}			
		}
		return "exist";
...
if(!function_exists('mbw_refresh_auth_cookie')){
	function mbw_refresh_auth_cookie(){
		...
		$user_access_token	= $mdb->get_var($mdb->prepare("select ".$mb_fields["users"]["fn_user_access_token"]." from `".$mb_admin_tables["users"]."` where `".$mb_fields["users"]["fn_user_id"]."`=%s",$user_id));
...
if(!function_exists('mbw_validate_auth_cookie')){
	function mbw_validate_auth_cookie(){
		...
		list($user_id, $expiration, $hmac, $user_mode) = $cookie_elements;
		$hash		= mbw_get_hash_key("cookie",$expiration);
		...
		if($mode=="today_visit"){	
			...
			if(mbw_get_cookie("mb_".$mode)==""){
				if(!empty($_SERVER['HTTP_REFERER'])){					
					...
					if(strpos(MBW_HOME_URL, $referer_host) === false){
