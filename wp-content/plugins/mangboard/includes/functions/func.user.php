	...
	function mbw_set_user_point($target,$action,$add_point=0,$user_pid="0",$user_name=""){
		...
		if(mbw_is_login() || !empty($user_pid)){		
			if($target=="comment"){
				...
				if($action=="delete")	$point		= $point*(-1);
				if($point<0){
					$sign			= "-";
					$point		= $point*(-1);
				}
...
if(!function_exists('mbw_get_user_url')){
	function mbw_get_user_url($type="",$mode=""){
		...
		$user_url			= array();
		$site_url				= mbw_get_ssl_url(MBW_SITE_URL);		
		if($user_mode=="MB"){			
			...
			if(mbw_get_option("user_form_mode")=="" || !function_exists('mbw_login_url_filter')){
				$user_url["register"]					= wp_registration_url();
				$user_url["lost_password"]			= wp_lostpassword_url();
				$user_url["login"]						= wp_login_url();
				$user_url["logout"]					= wp_logout_url();
				$user_url["user_info"]				= get_edit_profile_url();
			}else{
