...
Class MStore
{
	...
	public function init_board($name){
		$this->set_board_name($name);
		$this->set_board_options($name);
	}
	...
	public function set_board_options($board_name){		
		...
		if(empty($this->board_options)){
			...
			define("MBW_SKIN_PATH", $skin_path."skins/".$skin_name."/");
			define("MBW_SKIN_URL", $skin_url."skins/".$skin_name."/");

			$this->set_board_languages();

			//스킨 템플릿 파일 읽어오기
			if(is_file(MBW_SKIN_PATH."includes/skin-template.php")) require_once(MBW_SKIN_PATH."includes/skin-template.php");
			...
			if(is_file(MBW_SKIN_PATH."includes/skin-filters.php")) require_once(MBW_SKIN_PATH."includes/skin-filters.php");
			...
			else $model_path		= MBW_SKIN_PATH."includes/skin-model.php";
			...
			if(mbw_get_request_mode()=="Frontend"){
				if(is_dir(MBW_SKIN_PATH)){  //스킨 존재하는지 체크		
					$skin_settings_path		= MBW_SKIN_PATH."includes/skin-settings.php";
					...
					foreach($mb_api_urls as $key => $value){
						...
						if(is_file(MBW_SKIN_PATH."api/".$api_type."-".$file_name)){
	...
	public function is_login(){
		if($this->user_login) return true;
	...
	public function is_admin_page(){	
		...
		//back-end (admin-ajax)
		if($this->is_admin() && mbw_get_param("action")!="" && mbw_get_param("admin_page")=="true") return true; 
	...
	public function get_board_select_fields($data,$board_name=""){
		...
		global $mb_admin_tables,$mb_fields;
		if(!empty($board_name) && $this->board_name!=$board_name){
