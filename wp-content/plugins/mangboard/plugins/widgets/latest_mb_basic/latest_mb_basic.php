...
if(!function_exists('mbw_get_latest_mb_basic')){
	function mbw_get_latest_mb_basic($mode,$data){		
		...
		$latest_permalink		= $latest_permalink."mode=view&board_pid=";
		...
		$latest_html	= $latest_html.'<div'.$div_style.' class="mb-latest-basic mb-widget-'.$mode.$div_class.'">';
		if($mode!="sidebar" && !empty($title)) 
			$latest_html	= $latest_html.'<div class="mb-latest-title"><a href="'.get_permalink($post_id).'" title="'.esc_attr($title).'">'.$title.'</a></div>';

		$latest_html	= $latest_html.'<div class="mb-latest-box"><table  cellspacing="0" cellpadding="0" border="0" class="table table-latest">';
		$latest_html	= $latest_html.'<colgroup><col style="width:100%" /></colgroup>';
		$latest_html	= $latest_html.'<tbody>';
		...
		if(!empty($latest_list)){
			foreach($latest_list as $latest_item){
				...
				$latest_html	= $latest_html.'<tr><td class="mb-latest-item-title">';
				$latest_html	= $latest_html.'<a href="'.$latest_permalink.$latest_item[$board_field["fn_pid"]].'" title="'.esc_attr($latest_item[$board_field["fn_title"]]).'"><span>'.mbw_htmlspecialchars($post_title).'</span></a>';
				$latest_html	= $latest_html.'</td></tr>';
			}
		}else{
			$latest_html	= $latest_html.'<tr><td align="center" style="text-align:center;">'.__MM("MSG_LIST_ITEM_EMPTY").'</td></tr>';
		}

		$latest_html	= $latest_html."</tbody></table></div>";
		$latest_html	= $latest_html."</div></div>";
		return $latest_html;
	}
}
