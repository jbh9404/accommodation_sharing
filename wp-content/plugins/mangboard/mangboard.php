...
if ((! function_exists( 'add_filter' )) || (!is_admin() && (defined( 'DOING_AJAX' ) || defined('DOING_CRON')))){	
	return;
}
...
$auth_site_url		= str_replace(array("http://","https://"), "", $site_url);
...
function mbw_check_shortcode($posts){
	...
	foreach($posts as $post){	
		if(strpos($post->post_content, '['.MBW_SHORTCODE_BOARD) !== false){
			$post_content			= $post->post_content;
			$index1					= strpos($post_content,'['.MBW_SHORTCODE_BOARD." name=")+16;
			$index2					= strpos($post_content,"\"",$index1);
			$mb_board_name	= trim(substr($post_content,$index1,$index2-$index1));
