<?php
add_action('wp_ajax_nopriv_wpestate_send_valid_sms', 'wpestate_send_valid_sms');
...
add_action('wp_ajax_nopriv_wpestate_direct_pay_booking', 'custom_coupon_booking');
add_action('wp_ajax_wpestate_direct_pay_booking', 'custom_coupon_booking');

if (!function_exists('custom_coupon_booking')):
    function custom_coupon_booking()
    {
        /*
         * 나그네 서비스에서 쿠폰을 사용한 결제가 가능하도록 합니다.
         * 170913 - 김상결 작성
         */
        ...
        $couponListQuery = "select * from coupon_list where user_idx = '$user_idx' and coupon_used = '0'";
        ...
        if ($total_num > 0) {
            ...
            $invoice_no = intval($_POST['invoiceid']); // 청구번호
            $bookid = intval($_POST['book_id']); //예약번호
            ...
            $invoice_status_full = get_post_meta($invoice_no, 'invoice_status_full', 'no');
            if ($invoice_status_full == "confirmed") { //이미 결제완료됨
                echo 'already_finished';
            } else { // 결제가 되지 않은 상태
                // 정보 업데이트
                update_post_meta($invoice_no, 'balance', 0);
                update_post_meta($invoice_no, 'invoice_status_full', 'confirmed');
                add_post_meta($invoice_no, '_edit_lock', '1505298008:1');

                update_post_meta($bookid, 'booking_status_full', 'confirmed');

                $owner_idx = get_post_meta($bookid, 'owner_id', -1);

                //쿠폰 차감
                $couponCodeQuery = "select coupon_hash from coupon_list where user_idx = '$user_idx' and coupon_used = '0' limit 1";
                $couponCodeResult = mysqli_query($db_connection, $couponCodeQuery);

                $couponCode = mysqli_fetch_row($couponCodeResult)[0];

                $couponUseQuery = "update coupon_list set coupon_used = '1' where user_idx = '$user_idx' and coupon_hash = '$couponCode'";
                mysqli_query($db_connection, $couponUseQuery);

                //쿠폰 지급
                $currentdatetime = date('Y-m-d H:i:s');
                $couponUpdateQuery = "insert into coupon_list (coupon_idx, order_id, user_idx, coupon_hash, coupon_registered, coupon_used) values (NULL , '$bookid', '$owner_idx', '$couponCode','$currentdatetime' ,0)";
                mysqli_query($db_connection, $couponUpdateQuery);
    ...
    function wpestate_direct_pay_booking()
    {
        ...
        $userID = $current_user->ID;
        $user_email = $current_user->user_email;
        //$user_email         =   'skkim@didimstory.com' ;
...
if (!function_exists('wpestate_ajax_update_profile')):
    ...
    function wpestate_ajax_update_profile()
    {
        ...
        $usergender = sanitize_text_field(wp_kses($_POST['usergender'], $allowed_html));
        $userbirthday = sanitize_text_field(wp_kses($_POST['userbirthday'], $allowed_html));
        ...
        update_user_meta($userID, 'usergender', $usergender);
        update_user_meta($userID, 'userbirthday', $userbirthday);

        $agent_id = get_user_meta($userID, 'user_agent_id', true);

        wpestate_update_user_agent($agent_id, $firstname, $secondname, $useremail, $userphone, $userskype, $profile_image_url, $usermobile, $about_me, $profile_image_url_small, $userfacebook, $usertwitter, $userlinkedin, $userpinterest, $live_in, $i_speak, $login_name, $payment_info, $usergender, $userbirthday);
...
if (!function_exists('wpestate_update_user_agent')):
    function wpestate_update_user_agent($agent_id, $firstname, $secondname, $useremail, $userphone, $userskype, $profile_image_url, $usermobile, $about_me, $profile_image_url_small, $userfacebook, $usertwitter, $userlinkedin, $userpinterest, $live_in, $i_speak, $login_name, $payment_info, $usergender, $userbirthday)
    {
        ...
        update_post_meta($agent_id, 'usergender', $usergender);
        update_post_meta($agent_id, 'userbirthday', $userbirthday);
