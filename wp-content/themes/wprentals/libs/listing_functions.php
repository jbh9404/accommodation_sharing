    ...
    function estate_listing_price($post_id)
    {
        ...
        if (is_array($extra_pay_options) && !empty($extra_pay_options)) {
            ...
            foreach ($extra_pay_options as $key => $options) {
                ...
                $return_string .= '' . $options[0] . ': ' . $options[1] . ' ' . $options_array[$options[2]];
...
if (!function_exists('estate_listing_address')):
    function estate_listing_address($post_id)
    {
        ...
        $address_splited = explode(" ", $property_address);
        ...
        $return_string = '';
        if ($property_city != '') {
            $return_string .= '<div class="listing_detail list_detail_prop_city col-md-6"><span class="item_head">' . esc_html__('호스트의 도시') . ':</span> ' . $property_city . '</div>';
        }
        if ($property_address != '') {
            $return_string .= '<div class="listing_detail list_detail_prop_address col-md-6"><span class="item_head">' . esc_html__('호스트의 주소') . ':</span> ' .$address_splited[0]." ".$address_splited[1]." ".$address_splited[2]. '</div>';
        }
